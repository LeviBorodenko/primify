...
class PrimeImage(object):
    ...
    def _resize(self):
        ...
        if self.VERBOSE:
            print(
                f"Resized image to be {self.width}x{self.im.size[1]} pixels.")
    ...
    def numberise(self):
        ...
        self.CONVERTED_IMAGE_DATA = [str(self.ORDERED_DIGITS[-i])
                                     for i in self.im.getdata()]
    ...
    def find_next_prime(self):
        ...
        if self.VERBOSE:
            print(f"""Starting search for prime.
...
if __name__ == "__main__":
    ...
    parser.add_argument("--method", action="store",
                        type=int, choices=[0, 1, 2], dest="method",
    ...
    a = PrimeImage(image_path=args.image_path,
